## 0.9.25 (2024-08-16)

This was a version bump only for textfield to align it with other projects, there were no code changes.

## 0.10.1 (2024-04-23)

This was a version bump only for textfield to align it with other projects, there were no code changes.

## 0.10.0 (2024-04-23)

### 🚀 Features

- new icon lib (lucide)

### ❤️ Thank You

- PHIEKS
- pixelrick

## 0.9.24 (2024-04-17)

### 🩹 Fixes

- update storybook to 8.x

### ❤️ Thank You

- Jakob Sanne-Grubbström
- pixelrick

# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

## [0.9.23] (2024-04-10)

**Note:** Version bump only for package @mvds/textfield

## [0.9.22] (2024-04-09)

**Note:** Version bump only for package @mvds/textfield

## [0.9.21] (2024-04-03)

**Note:** Version bump only for package @mvds/textfield

## [0.9.20] (2024-04-03)

**Note:** Version bump only for package @mvds/textfield

## [0.9.19] (2024-04-03)

**Note:** Version bump only for package @mvds/textfield

## [0.9.18] (2024-04-03)

**Note:** Version bump only for package @mvds/textfield

## [0.9.17] (2024-04-03)

**Note:** Version bump only for package @mvds/textfield

## [0.9.16] (2024-02-29)

**Note:** Version bump only for package @mvds/textfield

## [0.9.15] (2024-02-29)

**Note:** Version bump only for package @mvds/textfield

## [0.9.14] (2024-02-29)

**Note:** Version bump only for package @mvds/textfield

## [0.9.13] (2024-02-29)

**Note:** Version bump only for package @mvds/textfield

## [0.9.12] (2024-02-29)

# 1.0.0-20240228-1507.1 (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.11] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.10] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.9] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.8] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.7] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.6] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.5] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.4] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.3] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.2] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

## [0.9.1] (2024-02-28)

**Note:** Version bump only for package @mvds/textfield

# [0.9.0] (2024-02-28)

### Bug Fixes

- set all tokens to camelCase )

# 1.0.0-20240226-1427.1 (2024-02-26)

### Features

- **textfield:** add required attribute, fix focused styles )
- **textfield:** fix password / other type validations )

# [0.8.0] (2024-02-28)

### Bug Fixes

- set all tokens to camelCase )

# 1.0.0-20240226-1427.1 (2024-02-26)

### Features

- **textfield:** add required attribute, fix focused styles )
- **textfield:** fix password / other type validations )

## [0.7.14] (2024-02-13)

**Note:** Version bump only for package @mvds/textfield

## [0.7.13] (2024-02-13)

**Note:** Version bump only for package @mvds/textfield

## [0.7.12] (2024-02-13)

**Note:** Version bump only for package @mvds/textfield

## [0.7.11] (2024-02-04)

**Note:** Version bump only for package @mvds/textfield

## [0.7.10] (2024-02-04)

**Note:** Version bump only for package @mvds/textfield

## [0.7.9] (2024-02-04)

**Note:** Version bump only for package @mvds/textfield

## [0.7.8] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.7] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.6] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.5] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.4] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.3] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.2] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

## [0.7.1] (2024-02-02)

**Note:** Version bump only for package @mvds/textfield

# [0.7.0] (2024-02-01)

### Features

- **textfield:** add disabled state )

# [0.6.0] (2024-02-01)

### Features

- **textfield:** add error icon )

## [0.5.3] (2024-01-31)

**Note:** Version bump only for package @mvds/textfield

## [0.5.2] (2024-01-31)

# 1.0.0-20240131-2202 (2024-01-31)

**Note:** Version bump only for package @mvds/textfield

## [0.5.2] (2024-01-31)

**Note:** Version bump only for package @mvds/textfield

## [0.5.1] (2024-01-31)

**Note:** Version bump only for package @mvds/textfield

# 0.5.0 (2024-01-31)

### Features

- **textfield:** add boilerplate starter pack for new component )

# 0.4.0 (2024-01-31)

### Features

- **textfield:** add boilerplate starter pack for new component )

# 0.3.0 (2024-01-31)

### Features

- **textfield:** add boilerplate starter pack for new component )

# 0.2.0 (2024-01-31)

### Features

- **textfield:** add boilerplate starter pack for new component )

# 0.1.0 (2024-01-31)

### Features

- **textfield:** add boilerplate starter pack for new component )
