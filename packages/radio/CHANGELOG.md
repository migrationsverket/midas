## 0.6.3 (2024-09-19)


### 🚀 Features

- **textarea:** new component


### 🩹 Fixes

- change node version on deploy

- new readme + doc build

- fix disable state text color and change px to rem

- **docs:** fix storybook link

- **radio:** change padding on label and helper text

- **docs:** update url

- **docs:** update url


### ❤️  Thank You

- jabir Khalil
- PHIEKS
- Wilhelm Hjelm

## 0.6.2 (2024-08-12)

This was a version bump only for radio to align it with other projects, there were no code changes.

## 1.1.2 (2024-04-26)

### 🩹 Fixes

- padding till radio komponent -mobil version-

- margin till error text i radio komponent -mobil version-

- **radio:** remove typo in template

- **radio:** remove border mobile

### ❤️ Thank You

- jabir khalil
- Jabir Saadi Khalil
- Jakob Sanne-Grubbström
- pixelrick

## 1.1.1 (2024-04-23)

This was a version bump only for radio to align it with other projects, there were no code changes.

## 1.1.0 (2024-04-23)

### 🚀 Features

- new icon lib (lucide)

### ❤️ Thank You

- pixelrick

## 1.0.2 (2024-04-23)

### 🩹 Fixes

- **radio:** minor style fixes and use heroicon import

### ❤️ Thank You

- PHIEKS
- pixelrick

## 1.0.1 (2024-04-17)

### 🩹 Fixes

- update storybook to 8.x

### ❤️ Thank You

- Jakob Sanne-Grubbström
- pixelrick

# 1.0.0 (2024-04-12)

This was a version bump only for radio to align it with other projects, there were no code changes.

# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

## [0.6.1] (2024-04-10)

**Note:** Version bump only for package @mvds/radio

# [0.6.0] (2024-04-10)

### Features

- **radio:** change builder to vite )
- **radio:** change padding )

## [0.5.22] (2024-04-09)

**Note:** Version bump only for package @mvds/radio

## [0.5.21] (2024-04-03)

**Note:** Version bump only for package @mvds/radio

## [0.5.20] (2024-04-03)

**Note:** Version bump only for package @mvds/radio

## [0.5.19] (2024-04-03)

**Note:** Version bump only for package @mvds/radio

## [0.5.18] (2024-04-03)

**Note:** Version bump only for package @mvds/radio

## [0.5.17] (2024-04-03)

**Note:** Version bump only for package @mvds/radio

## [0.5.16] (2024-02-29)

**Note:** Version bump only for package @mvds/radio

## [0.5.15] (2024-02-29)

**Note:** Version bump only for package @mvds/radio

## [0.5.14] (2024-02-29)

**Note:** Version bump only for package @mvds/radio

## [0.5.13] (2024-02-29)

**Note:** Version bump only for package @mvds/radio

## [0.5.12] (2024-02-29)

# 1.0.0-20240228-1507.1 (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.11] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.10] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.9] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.8] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.7] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.6] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.5] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.4] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.3] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.2] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

## [0.5.1] (2024-02-28)

**Note:** Version bump only for package @mvds/radio

# [0.5.0] (2024-02-28)

### Bug Fixes

- set all tokens to camelCase )

# 1.0.0-20240227-1016.1 (2024-02-27)

# 1.0.0-20240226-1427.1 (2024-02-26)

### Features

- **radio:** fix mobile focus )

# 1.0.0-20240213-1522.1 (2024-02-13)

### Features

- **radio:** add active state color and fix focus outline )

# [0.4.0] (2024-02-28)

### Bug Fixes

- set all tokens to camelCase )

# 1.0.0-20240227-1016.1 (2024-02-27)

# 1.0.0-20240226-1427.1 (2024-02-26)

### Features

- **radio:** fix mobile focus )

# 1.0.0-20240213-1522.1 (2024-02-13)

### Features

- **radio:** add active state color and fix focus outline )

## [0.3.13] (2024-02-13)

**Note:** Version bump only for package @mvds/radio

## [0.3.12] (2024-02-13)

**Note:** Version bump only for package @mvds/radio

## [0.3.11] (2024-02-13)

**Note:** Version bump only for package @mvds/radio

## [0.3.10] (2024-02-04)

**Note:** Version bump only for package @mvds/radio

## [0.3.9] (2024-02-04)

**Note:** Version bump only for package @mvds/radio

## [0.3.8] (2024-02-04)

**Note:** Version bump only for package @mvds/radio

## [0.3.7] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.6] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.5] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.4] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.3] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.2] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

## [0.3.1] (2024-02-02)

**Note:** Version bump only for package @mvds/radio

# [0.3.0] (2024-02-02)

### Features

- **radio:** add basic mobile design for radio )

## [0.2.12] (2024-02-01)

**Note:** Version bump only for package @mvds/radio

## [0.2.11] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.10] (2024-01-31)

# 1.0.0-20240131-2202 (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.9] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.8] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.7] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.6] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.5] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.4] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.3] (2024-01-31)

**Note:** Version bump only for package @mvds/radio

## [0.2.2] (2024-01-23)

**Note:** Version bump only for package @mvds/radio

## 0.2.1 (2024-01-23)

**Note:** Version bump only for package @mvds/radio
