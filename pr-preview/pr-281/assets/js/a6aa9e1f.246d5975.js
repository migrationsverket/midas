...
/** Added this for a better output, will see how this plays out */function useMultiSelectState(props){var _useState=(0,react.useState)(false),isFocused=_useState[0],setFocused=_useState[1];// TODO: What is our value here?
var validationState=(0,useFormValidationState/* useFormValidationState */.KZ)(Object.assign({},props,{value:'1'}));var triggerState=(0,useMenuTriggerState/* useMenuTriggerState */.I)(props);var listState=useMultiSelectListState(Object.assign({},props,{onSelectionChange:function onSelectionChange(keys){var onSelectionChange=props.onSelectionChange,selectionMode=props.selectionMode;if(onSelectionChange!=null){if(keys==='all'){// This may change back to "all" once we will implement async loading of additional
...
if(selectionMode==='single'){triggerState.close();}}}));return Object.assign({},listState,triggerState,{close:function close(){triggerState.close();},open:function open(){// Don't open if the collection is empty.
 ...
 */function useHiddenMultiSelect(props,state,triggerRef){// const data = selectData.get(state) || {}
var autoComplete=props.autoComplete,_props$name=props.name,name=_props$name===void 0?'derp':_props$name,_props$isDisabled=props.isDisabled,isDisabled=_props$isDisabled===void 0?false:_props$isDisabled;var validationBehavior='native';var isRequired=true;// const { validationBehavior, isRequired } = data
var _useVisuallyHidden=(0,VisuallyHidden/* useVisuallyHidden */.B)(),visuallyHiddenProps=_useVisuallyHidden.visuallyHiddenProps;(0,useFormReset/* useFormReset */.F)(props.selectRef,state.selectedKeys,state.setSelectedKeys);(0,useFormValidation/* useFormValidation */.X)({validationBehavior:validationBehavior,focus:function focus(){var _triggerRef$current;return(_triggerRef$current=triggerRef.current)==null?void 0:_triggerRef$current.focus();}},state,props.selectRef);// In Safari, the <select> cannot have `display: none` or `hidden` for autofill to work.
...
'data-react-aria-prevent-focus':true,'data-a11y-ignore':'aria-hidden-focus'}),inputProps:{style:{display:'none'}},selectProps:{tabIndex:-1,autoComplete:autoComplete,disabled:isDisabled,required:validationBehavior==='native'&&isRequired,name:name,onChange:function onChange(e){return state.setSelectedKeys(e.target.value);}}};}/**
