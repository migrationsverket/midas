...
'use client';var _excluded=["tabs","label","children","className"];// Define a type for children elements that can accept an id prop
var Tabs=function Tabs(_ref){var tabs=_ref.tabs,label=_ref.label,children=_ref.children,className=_ref.className,rest=(0,objectWithoutPropertiesLoose/* default */.A)(_ref,_excluded);var _useObserveElement=(0,useObserveElement/* default */.A)(typeof document!=='undefined'&&document.body||null,true),bodyWidth=_useObserveElement.width;var orientation=bodyWidth>=tokens.windowSizes.md?'horizontal':'vertical';var childrenArray=react.Children.toArray(children);// Check if the number of children matches the number of tabs
if(childrenArray.length!==tabs.length){console.error("The number of children must match the number of tabs. Children: "+childrenArray.length+" Tabs: "+tabs.length);return null;}// Create a map of tab titles to their corresponding content
var tabContentMap=childrenArray.reduce(function(acc,child,index){if(/*#__PURE__*/react.isValidElement(child)){var title=tabs[index];if(title){acc[title]=/*#__PURE__*/react.cloneElement(child,{id:title});}}return acc;},{});return/*#__PURE__*/(0,jsx_runtime.jsxs)(dist_Tabs/* Tabs */.tU,Object.assign({orientation:orientation,className:(0,clsx/* default */.A)(Tabs_module.container,className)},rest,{defaultSelectedKey:rest.defaultSelected||rest.defaultSelectedKey,children:[/*#__PURE__*/(0,jsx_runtime.jsx)(dist_Tabs/* TabList */.wb,{"aria-label":label,className:Tabs_module.list,children:tabs.map(function(tab){return/*#__PURE__*/(0,jsx_runtime.jsx)(dist_Tabs/* Tab */.oz,{id:tab,className:Tabs_module.listItem,children:tab},tab);})}),tabs.map(function(tab){return/*#__PURE__*/(0,jsx_runtime.jsx)(dist_Tabs/* TabPanel */.Kp,{id:tab,className:Tabs_module.panel,children:tabContentMap[tab]},tab);})]}));};
...
function useObserveElement(element,includePadding){if(includePadding===void 0){includePadding=false;}var debounce=function debounce(callback,delay){var timeout;return function(entries,observer){clearTimeout(timeout);timeout=setTimeout(function(){callback(entries,observer);},delay);};};var _useState=(0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(0),width=_useState[0],setWidth=_useState[1];var _useState2=(0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(0),height=_useState2[0],setHeight=_useState2[1];(0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(function(){if(!element)return;var resizeObserver=new ResizeObserver(debounce(function(entries){var _ref=includePadding?entries[0].borderBoxSize[0]:entries[0].contentBoxSize[0],inlineSize=_ref.inlineSize,blockSize=_ref.blockSize;setWidth(inlineSize);setHeight(blockSize);},500));resizeObserver.observe(element);return function(){resizeObserver.disconnect();};},[element,includePadding]);return{width:width,height:height};}
