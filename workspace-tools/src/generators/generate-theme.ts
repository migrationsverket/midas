...
export default async function (tree: Tree, schema: any) {
  ...
  let cssOutput =
    '/* THIS FILE IS AUTOGENERATED FROM tokens.ts. DO NOT EDIT MANUALLY. */\n\n:root {\n'
  ...
  function formatCssValue(value, cssVarName) {
    if (typeof value === 'number') {
      ...
      if (
        cssVarName.startsWith('--z-index-') ||
        cssVarName.startsWith('--typography-weight-')
      ) {
        return value
      }
    ...
    if (
      cssVarName === '--typography-font-family' &&
      typeof value === 'string' &&
      value[0] !== "'" &&
      value[0] !== '"'
    ) {
    ...
    if (
      cssVarName.startsWith('--breakpoint-') &&
      typeof value === 'string' &&
      value.startsWith('(')
    ) {
  ...
  const formattedCssOutput = await prettier.format(cssOutput, {
    ...prettierConfig,
    parser: 'css',
    singleQuote: true,
  }) // Explicitly set singleQuote
  try {
