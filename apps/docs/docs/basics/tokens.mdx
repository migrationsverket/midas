...
Vissa av våra tokens är kopplade till specifika lager, vilket indikeras av ett nummer i slutet av namnet: antingen 01 eller 02. Tokens som slutar på 01 (t.ex. `field01`) tillhör lager 1 och tokens som slutar på 02 tillhör lager 2. Alla tokens med samma nummer används tillsammans inom samma lager. Numreringen ger också lagerstrukturen: tokens som slutar på 01 (t.ex. `field01`) ligger på `background` (lager 0) och tokens som slutar på 02 ligger på `layer01`.
