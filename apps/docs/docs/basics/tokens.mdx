...
Vi har bytt system för att hantera tokens till [style-dictionary](https://styledictionary.com/) och passat på att
uppdatera en del variabelnamn, både CSS-variabler och typescriptvariabler. Vi fortsätter att supporta `semantics` och
`theme` ytterligare någon version men var förberedda på att detta kommer att bli borttaget i en framtida version.
Nytt system för tokens finns dokumenterat i [tokens](/basics/tokens-v2/).
:::

Här kan du läsa om hur vi använder färger i Midas. Vi använder i huvudsak en gråskala och en blå skala som grund för våra
...
export const TokenGroup = ({ children }) => {
  return (
    <Grid className='card-guidelines'>
      {children}
    </Grid>
...
export const Token = ({token, desc}) => {
    ...
    return (
        <GridItem size={12} style={{ borderBottom: '1px solid var(--ifm-color-emphasis-200)', paddingBottom: '1rem' }}>
            <Grid>
                <GridItem size={{xs: 12, sm: 12, md: 3}}>
                    <CodeBlock className="margin-0">{token}</CodeBlock>
                </GridItem>
                <GridItem size={{xs: 12, sm: 12, md: 3}}>
                    <small>{desc}</small>
                </GridItem>
                <GridItem size={{xs: 12, sm: 12, md: 6}} style={{alignItems: 'flex-end'}}>
