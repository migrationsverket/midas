...
Strängen räcker i de flesta fall. Funktionen är användbar när du kombinerar flera HTML constraint-regler och vill ge användaren ett specifikt meddelande beroende på _varför_ fältet är ogiltigt. Funktionen tar emot ett `ValidationResult`-objekt med `validationDetails` som berättar exakt vilket krav som bröts:
