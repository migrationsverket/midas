...
ColorSchemeSwitch justerar `data-color-scheme` på `:root` som standard. Du kan justera vilken selector som ska användas
...
Komponenten accepterar tre värden: `light`, `dark` och `light dark` — enbart ljust läge, enbart mörkt läge eller
att följa systemets inställningar. Default är `light dark`. Om du vill att ett specifikt tema är förvalt
kan du skicka in standardvärdet direkt i `defaultScheme`.
