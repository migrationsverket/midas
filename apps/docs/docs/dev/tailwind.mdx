...
### Skapa utilities med @theme (rekommenderat)

För att automatiskt få tillgång till alla Midas tokens som Tailwind utilities, importera den färdiga `tailwind-theme.css` filen:
...
Nu kan du använda alla Midas tokens direkt som Tailwind utilities:
...
:::tip
`tailwind-theme.css` genereras automatiskt från dina JSON tokens via Style Dictionary.
Alla `--midas-*` variabler blir tillgängliga utan prefixet, t.ex. `--midas-color-blue-100` → `bg-blue-100`.
:::

### Anpassa @theme

Om du bara vill ha vissa tokens eller vill skapa egna namn, kan du definiera ditt eget `@theme`-block:
