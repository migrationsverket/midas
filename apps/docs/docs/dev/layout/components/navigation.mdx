...
Navigationskomponent som används både i [Sidebar](/dev/layout/components/sidebar), [Navbar](/dev/layout/components/navbar) och [MobileMenu](/dev/layout/components/mobilemenu) men med olika utseende beroende på kontext. Navigation anpassar automatiskt utseendet efter var den befinner sig. Den består av tre komponenter: `Navigation`, `NavigationLink` och `NavigationSection`.
