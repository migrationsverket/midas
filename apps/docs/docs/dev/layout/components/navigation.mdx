...
Navigationskomponent som används både i [Sidebar](/dev/layout/components/sidebar), [Navbar](/dev/layout/components/navbar) och [MobileMenu](/dev/layout/components/header/#mobilemenu) men med olika utseende beroende på kontext. Navigation anpassar automatiskt utseendet efter var den befinner sig.
...
För att skapa en navigeringsmeny behövs tre komponenter: `Navigation`, `NavigationItem` och `NavigationLink`. `Navigation` är en wrapper som renderar ett `<nav>`-element med `<ul>` inuti. Den stödjer både statisk och dynamisk rendering via Collections. `NavigationItem` är en wrapper som renderar ett `<li>`-element och används för att gruppera `NavigationLinks` och eventuella undermenyer.

`NavigationLink` är själva länken som renderas som `<a>`-element och representerar ett menyalternativ i navigationen. De ska alltid ha både ikon och etikett.
