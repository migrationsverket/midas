...
Den externa varianten (navbar längst ned på mobil) hanteras automatiskt av det nya paketet. Lägg till `Navbar` i din struktur — `Sidebar` och `Navbar` döljer sig automatiskt beroende på skärmstorlek.
