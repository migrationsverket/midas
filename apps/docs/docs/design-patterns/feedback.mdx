...
Vi använder handling för att beskriva ett medvetet initiativ av användaren för att få systemet att göra något. Handlingar utförs typiskt genom knappar eller menyalternativ.

### Typer av Handlingar

Vi skiljer mellan tre sorters handlingar:

- [Tillgängliga handlingar](#tillgängliga-handlingar): handlingar som alltid är möjliga att utföra och som systemet sedan utvärderar
- [Villkorade handlingar](#förutsättningsstyrda-handlingar): handlingar som kräver specifika förutsättningar för att kunna utföras
- [Otillgängliga handlingar](#otillgänglig-handling): handlingar som inte är möjliga att utföra
...
2. Handlingstypen avgör om och när handlingen kan utföras
3. Ett återkopplingsmönster avgör hur systemet svarar användaren

## Tillgängliga handlingar

Denna sortens handling utförs alltid när den initieras av användaren. Därefter utvärderar systemet resultatet genom validering eller affärslogik. Fel eller saknade förutsättningar kommuniceras efter användarens försök att genomföra handling, inte genom att hindra handlingen.

### Användning

- När användaren skickar in ett formulär
- När användaren sparar data
...
- När det inte är meningsfullt eller tillåtet att utföra handlingen om inte kraven är uppfyllda
...
Ovanstående krav uppfyllas på följande två sätt:

Alternativ 1: Visa en tooltip på hover och fokus. För att göra detta tillgängligt för skärmläsaranvändare behöver vi även sätta trigger som `aria-disabled` för att indikera för skärmläsaranvändare att knappen är inaktiv

<InactiveButtonExample />

Alternativ 2: Visa en infobanner eller en modal vid klick. Detta är lämpligt när det finns flera krav som måste uppfyllas eller när kraven är mer komplexa.
...
:::warning
I våra externa system ska det inte finnas några otillgängliga handlingar.
:::

En otillgänglig åtgärd är en handling som användaren inte kan utföra i aktuell kontext och där användaren själv inte kan uppfylla kraven för att handlingen ska vara möjlig.
...
Vi använder `isDisabled` för att indikera att en handling är otillgänglig. Enligt ovanstånde krav måste vi även ha en förklaring i direkt anslutning till triggerkomponenten.
