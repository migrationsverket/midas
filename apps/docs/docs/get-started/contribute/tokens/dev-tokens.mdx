...
Råa palettfärger som semantiska tokens refererar till. Används exklusivt via `support.*` (Toast, InfoBanner, invalida formulärfält).
...
Dedikerade primitiva paletter för Tag-komponentens färgvarianter. Varje palett har tre steg: `.20` (ljus bakgrund), `.60` (kantlinje), `.180` (mörk bakgrund).

<ColorPaletteTable tokens={{
  sky: tokenDictionary.color.sky,
  mint: tokenDictionary.color.mint,
  cream: tokenDictionary.color.cream,
  teal: tokenDictionary.color.teal,
  lagoon: tokenDictionary.color.lagoon,
  lavender: tokenDictionary.color.lavender,
  peach: tokenDictionary.color.peach,
  pippin: tokenDictionary.color.pippin,
}} />
...
Semantiska färgtokens för Tag-komponenten. Varje variant har ett dedikerat primitivt palett — `sky`, `mint`, `cream`, `teal`, `lagoon`, `lavender`, `peach`, `pippin`. Används i: Tag.

<TokenList tokens={tokenDictionary} filter="tag" />
