...
[Storybook som testplattform](https://storybook.js.org/docs/writing-tests).
...
Branch `main` representerar alltid den publicerade versionen av paketet [@midas/components](https://www.npmjs.com/package/@midas-ds/components) samt
korrekt version av dokumentationen på vår [dokumentationswebb](https://designsystem.migrationsverket.se/). Branch `dev` är arbetsversion och innehåller
alla features och fixar som har passerat test men ännu inte är släppta. Feature branches som har en PR publiceras på `designsystem.migrationsverket.se/pr-preview/pr-xxx/` som en del i arbetsprocessen.
