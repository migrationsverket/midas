...
Flex har inbyggda marginaler som anpassar sig efter skärmstorlek. Marginalerna är inställda på att ge en bra balans mellan innehåll och utrymme runtom. Dessa yttre marginaler går att stänga av med `noMargins` om du vill ha ett rutnät utan marginaler.
...
Grid används för att strukturera upp en sida och placera olika saker i kolumner. Med gridkomponenten får du med dig ovan
justeringar för responsivitet samt mellanrum mellan kolumner.

### Så här använder du Grid

Använd `<Grid></Grid>` som komponent och justera direkt underliggande komponenter med inställningar i Grid. Justera med barnkomponenten
`<GridItem size="antal-kolumner"></GridItem>`.
