...
Använd `value` och `onChange` för att styra det valda värdet i ComboBox.
...
För närvarande stödjer inte vår ComboBox flerval. Använd [Midas CSS för react-select](/dev/react-select) för detta.
