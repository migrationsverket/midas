...
```bash npm2yarn
npm install @migrationsverket/toast
```
...
```tsx
import { ToastProvider, MidasToastState } from '@migrationsverket/toast';
<ToastProvider>
...
```tsx
import { GlobalToastRegion, toastQueue } from '@mmigrationsverketvds/toast';
<GlobalToastRegion />
//...
<Button onPress={
