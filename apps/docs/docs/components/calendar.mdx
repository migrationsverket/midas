---
title: Calendar
description: Använd en kalender när användaren ska välja ett datum eller visualisera ett datum valt på annat sätt.
---

import { PropTable } from '@site/src/components/PropsTable'
import { Calendar } from '@midas-ds/components'
import { ComponentHeader } from '@site/src/components/getComponentMetaData'
import { RangeCalendarExample } from '@site/src/components/examples/calendar/CalendarExamples'

<ComponentHeader
  name='Calendar'
  friendlyName='Kalender'
  overrideHeadlessLink='https://react-spectrum.adobe.com/react-aria/Calendar.html'
/>

Fristående kalenderkomponent

```tsx
import { Calendar } from '@midas-ds/components'
```

```tsx
<Calendar aria-label='Kalender' />
```

<div
  className='card'
  style={{ display: 'block' }}
>
  <Calendar aria-label='Kalender' />
</div>

## Användning

Kalender kan användas när användaren ska välja ett datum eller visualisera ett valt datum. Normalt sett
är [DatePicker](/components/form/date-picker) standardvalet men kalendern kan användas när inputfältet inte
behövs, inte ligger i direkt anslutning till kalendern, eller om kalendern inte ska presenteras som en popover.

### Calendar

Kalendern accepterar samma props som [DatePicker](/components/form/date-picker) och controlled value sätts med `value` respektive `onChange`,
uncontrolled value sätts med `defaultValue`.

### RangeCalendar

Samma props som [DateRangePicker](/components/form/date-picker#daterangepicker), exempel nedan med controlled value:

```tsx
import { RangeCalendar } from '@midas-ds/components'
import { DateValue } from 'react-aria-components'

export const RangeCalendarExample = () => {
  const [selected, setSelected] = React.useState<{
    start: DateValue
    end: DateValue
  } | null>(null)

  return (
    <>
      <RangeCalendar
        aria-label='Kalender'
        value={selected}
        onChange={setSelected}
      />
      <pre>
        Valda datum:{' '}
        {selected &&
          `${selected?.start.toString()}
           till: ${selected?.end.toString()}`}
      </pre>
    </>
  )
}
```

<div
  className='card'
  style={{ display: 'block' }}
>
  <RangeCalendarExample aria-label='Kalender' />
</div>

## API

<PropTable name='Calendar' />
````
