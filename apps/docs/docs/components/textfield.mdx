...
Inmatningsfält när användaren ska fylla i kortare information, tex namn, personnummer eller mejladress. För längre inmatning används [TextArea](/components/textarea).
...
### inputMode och autoComplete

:::tip Mobilvänlig inmatning

Använd `inputMode` för att visa rätt virtuellt tangentbord på mobil, och `autoComplete` för att aktivera autofyll. Se exemplen nedan för vanliga kombinationer.

:::

`inputMode` kan användas för att ge mobilanvändare rätt tangentbord.
`inputMode="email"` visar ett virtuellt tangentbord som är anpassat för mejladressinmatning.

`autoComplete` kan användas för att aktivera autofyll för både desktop och mobil.

Använd en kombination av `type`, `inputMode` och `autoComplete` för validering, korrekt virtuellt tangentbord och autofyll.
...
Använd `inputMode="numeric"` för att visa ett numeriskt tangentbord på mobil utan att aktivera inbyggd validering av fältet.
...
Se [MDN — inputmode](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/inputmode) och [MDN — autocomplete](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/autocomplete#values) för alla tillgängliga värden.
