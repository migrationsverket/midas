...
export const Example = props => {
  return (
    <LiveCodeBlock
      ...
      {...props}
    >
      {`<p>Varannan vecka släpper vi en ny release, vilket du kan <Link href="/changelog">Läsa mer om här.</Link></p>`}
    </LiveCodeBlock>
  )
}
...
<LiveCodeBlock scope={{ Link }}>
  {`<p>Designsystemet utvecklas ständigt, <Link href="/changelog">vilket du kan läsa mer om här i vår releasefeed.</Link> Varannan vecka kommer det ett nytt släpp med buggfixar, nya komponenter eller nya funktioner på befintliga komponenter.</p>`}
</LiveCodeBlock>
