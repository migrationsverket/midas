---
title: Link
description: Komponent som skapar en länk
---

import { PropTable } from '@site/src/components/propsTable'
import { ComponentHeader } from '@site/src/components/getComponentMetaData'
import { Link } from '@midas-ds/components'
import { semantic } from '@midas-ds/components/theme'
import LiveCodeBlock from '@site/src/components/CodeBlock/CodeBlock'

export const r = (Math.random() + 1).toString(36).substring(7)

export const Example = props => {
  return (
    <LiveCodeBlock
      scope={{ Link }}
      {...props}
    >
      {`<p>Varannan vecka släpper vi en ny release, vilket du kan <Link href="/blog?v=${r}">läsa mer om här.</Link></p>`}
    </LiveCodeBlock>
  )
}

<ComponentHeader
  name={'Link'}
  friendlyName={'Länk'}
/>

Komponenten Link skapar en länk som kan vara i ett textstycke eller fristående.

<Example hideCode />

## Installation

```bash npm2yarn
npm install @midas-ds/components
```

```tsx
import { Link } from '@midas-ds/components'
```

<Example hideExample />

## Varianter

### Länk

<LiveCodeBlock scope={{ Link }}>
  {`<p>Designsystemet utvecklas ständigt, <Link href="/blog?v=${r}">vilket du kan läsa mer om här i vår releasefeed.</Link> Varannan vecka kommer det ett nytt släpp med buggfixar, nya komponenter eller nya funktioner på befintliga komponenter.</p>`}
</LiveCodeBlock>

### Fristående

Använd `standalone` för att använda komponenten som en fristående länk under t.ex ett textblock.

<LiveCodeBlock scope={{ Link }}>
  {`<>
    <p>Designsystemet utvecklas ständigt. Varannan vecka kommer det ett nytt släpp med buggfixar, nya komponenter eller nya funktioner på befintliga komponenter.</p>
    <Link
        standalone
        href="/blog?v=${r}"
    >
        Läs mer om designsystemets nyheter
    </Link>
</>`}
</LiveCodeBlock>

### "Utsträckt"

Använd `stretched` för att låta hela förälderelementet vara klickbart till länken. Föräldern måste ha `position: relative;` för att länkens klickyta inte ska gå för långt.

<LiveCodeBlock scope={{ Link, semantic }}>
  {`<div
      style={{
        position: 'relative',
        background: semantic.layer01,
        padding: '1rem'
      }}
    >
    <p>Designsystemet utvecklas ständigt. Varannan vecka kommer det ett nytt släpp med buggfixar, nya komponenter eller nya funktioner på befintliga komponenter. Om du håller muspekaren här så kan du klicka på länken över hela den gråa ytan.</p>
    <Link
        standalone
        stretched
        href="/blog?v=${r}"
    >
        Läs mer om designsystemets nyheter
    </Link>
</div>`}
</LiveCodeBlock>

## Riktlinjer

### Länktext

- Placera de viktigast orden först i länken. Det gör det enklare att hitta länken när användaren skannar av en sidan och förstå vad länken innebär.
- Var tydlig med var länken leder. Var beskrivande i vart den leder eller vad som händer när användaren klickar på den.
- Var specifik och tydlig med länkens syfte. Användaren ska förstå vad länken innebär även utan kontext (utan att behöva läsa all annan text). Skriv inte "Klicka här" eller "Läs mer" eftersom det inte hjälper användaren att särskilja länkar eller förstå dess syfte.
- Länkarna ska gå att särskilja. Använd samma text om länken leder till samma sida. En unik länk för varje unik destination.
- Skriv ut formatet om länken leder till ett dokument, till exempel pdf eller doc Exempel: Verksamhetsplan 2025 (pdf)
- Placera eventuell beskrivning av länken innan själva länken och i direkt anslutning till länken, i samma mening, stycke, listobjekt eller tabellcell som är nära förknippad med länken.

### Val av komponent

Se mönstret [Knappar och länkar](/design-patterns/buttons-and-links)

## API

<PropTable name='Link' />
