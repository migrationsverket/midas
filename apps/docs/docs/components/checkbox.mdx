...
Kryssrutor används för att låta användaren välja inget, ett eller flera alternativ. Om det endast ska gå att välja ett objekt i listan är det en [Radioknapp](./radio.mdx) som ska användas. Om det är fler alternativ än fyra (4) används med fördel [Select](./select.mdx) istället.
