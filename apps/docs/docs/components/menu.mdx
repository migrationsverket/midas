...
import { OnActionExample } from '@site/src/components/examples/menu/MenuExamples'
...
Använd `onAction` på `Menu` för att lyssna på vilket menyval användaren väljer. Callbacken tar emot `id`-värdet från det valda `MenuItem`-elementet — se till att sätta ett `id` på varje `MenuItem`.
