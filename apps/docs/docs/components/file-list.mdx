...
`progress` är bara ett tal du själv räknar fram och skickar in — komponenten vet inget om hur uppladdningen
sker. `fetch` kan inte rapportera uppladdningsförlopp alls; använd `XMLHttpRequest` (`upload.onprogress`) om
du vill visa ett bestämt förlopp. Går det inte att räkna ut ett tal, utelämna `progress` för ett obestämt
förlopp istället.
:::
...
`onCancel` och `onDelete` är bara callbacks — komponenten anropar dem när knappen trycks, men gör
