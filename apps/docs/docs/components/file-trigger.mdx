...
Använd FileTrigger tillsammans med [DropZone](/components/dropzone) för att tillåta val av filer via drag & drop.
DropZone är en generell yta för drag & drop och dokumenteras separat — se dess [egen sida](/components/dropzone)
...
som går att välja. Se [DropZone: Validering](/components/dropzone#validering) för filtypsbegränsning som även gäller
drag & drop.
